...
const REACT_FLOW_PRO_EXAMPLES = [
  { key: 'auto-layout', label: 'Auto Layout', zip: 'auto-layout-pro-example.zip' },
  { key: 'collaborative', label: 'Collaborative', zip: 'collaborative-pro-example.zip' },
  { key: 'copy-paste', label: 'Copy Paste', zip: 'copy-paste-pro-example.zip' },
  { key: 'dynamic-layouting', label: 'Dynamic Layouting', zip: 'dynamic-layouting-pro-example.zip' },
  { key: 'editable-edge', label: 'Editable Edge', zip: 'editable-edge-pro-example.zip' },
  { key: 'expand-collapse', label: 'Expand Collapse', zip: 'expand-collapse-pro-example.zip' },
  { key: 'force-layout', label: 'Force Layout', zip: 'force-layout-pro-example.zip' },
  { key: 'freehand-draw', label: 'Freehand Draw', zip: 'freehand-draw-pro-example.zip' },
  { key: 'helper-lines', label: 'Helper Lines', zip: 'helper-lines-pro-example.zip' },
  { key: 'libavoid-edge-routing', label: 'Libavoid Edge Routing', zip: 'libavoid-edge-routing-pro-example.zip' },
  { key: 'node-position-animation', label: 'Node Position Animation', zip: 'node-position-animation-pro-example.zip' },
  { key: 'parent-child-relation', label: 'Parent Child Relation', zip: 'parent-child-relation-pro-example.zip' },
  { key: 'remove-attribution', label: 'Remove Attribution', zip: 'remove-attribution-pro-example.zip' },
  { key: 'selection-grouping', label: 'Selection Grouping', zip: 'selection-grouping-pro-example.zip' },
  { key: 'server-side-image-creation', label: 'Server Side Image Creation', zip: 'server-side-image-creation-pro-example.zip' },
  { key: 'shapes', label: 'Shapes', zip: 'shapes-pro-example.zip' },
  { key: 'undo-redo', label: 'Undo Redo', zip: 'undo-redo-pro-example.zip' },
];

// Guard against accidental duplicate entries so the coverage panel remains accurate.
const hasDuplicateProFeatureEntries = (() => {
  const keys = new Set();
  const zips = new Set();
  for (const feature of REACT_FLOW_PRO_EXAMPLES) {
    if (keys.has(feature.key) || zips.has(feature.zip)) return true;
    keys.add(feature.key);
    zips.add(feature.zip);
  }
  return false;
})();
...
function TBtn({ onClick, disabled, active, danger, title, children, className = '' }) {
  const base = 'inline-flex items-center gap-1 rounded-lg px-3 py-1.5 text-[11px] font-semibold leading-none transition-all duration-100 focus:outline-none disabled:opacity-35 disabled:cursor-not-allowed';
  const style = active
  ...
  return (
    <button type="button" onClick={onClick} disabled={disabled} title={title} className={`${base} ${style} ${className}`}>
      {children}
    </button>
  );
}
...
function TDivider() {
  return <div className="h-4 w-px bg-white/[0.09] mx-0.5 flex-shrink-0" />;
}
...
function ProRuntimeBridge({ onViewport, onNodesReady, onConnecting, onApi }) {
  ...
  useEffect(() => {
    onNodesReady?.(nodesReady);
  }, [nodesReady, onNodesReady]);

  useEffect(() => {
    onConnecting?.(Boolean(connection?.inProgress));
  }, [connection?.inProgress, onConnecting]);

  return null;
}

function IBtn({ onClick, active, disabled, title, children, danger, activeColor }) {
  const ac = activeColor || '#38bdf8';
  return (
    <button
      type="button"
      onClick={onClick}
      ...
      style={{
        ...
        background: active ? `${ac}22` : danger ? 'rgba(239,68,68,.1)' : 'rgba(255,255,255,.04)',
        borderColor: active ? `${ac}88` : danger ? 'rgba(239,68,68,.4)' : 'rgba(255,255,255,.1)',
        color: active ? ac : danger ? '#f87171' : '#94a3b8',
        boxShadow: active ? '0 0 0 1.5px rgba(74,222,128,0.55)' : 'none',
      }}
    >
      {children}
    </button>
...
function UnifiedBuilderToolbar({
  interactionMode,
  setInteractionMode,
  isLocked,
  onToggleLock,
      ...
      <div style={{ padding: '6px 10px 5px', display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        {showProPanel && (
          <div style={{
            ...
            <div style={{ gridColumn: '1/-1', fontSize: 9, fontWeight: 700, color: '#4ade80', letterSpacing: '.6px', textTransform: 'uppercase', marginBottom: 4 }}>
              React Flow Pro — 17 / 17 features active ✅
            </div>
            {[
              ['⊞', 'Auto Layout',          'Layout button — ELK top-down AP flow order'],
              ['↩', 'Undo / Redo',           'Undo + Redo buttons · Ctrl+Z / Ctrl+Y'],
              ['⧉', 'Copy / Paste',          'Copy + Paste buttons · Ctrl+C / Ctrl+V'],
              ['↝', 'Collaborative',         '● Live dot — real-time via Yjs + BroadcastChannel'],
              ['⌒', 'Editable Edge',         'Click a transition → drag the midpoint bend handle'],
              ['▣', 'Selection Grouping',    'Drag to multi-select → Group button'],
              ['▼', 'Expand / Collapse',     'Group + Collapse / Expand buttons'],
              ['⟳', 'Force Layout',          'Force button — D3 physics, even spacing'],
              ['✏', 'Freehand Draw',         'Draw mode → sketch annotations on canvas'],
              ['⌇', 'Helper Lines',          'Drag any node — alignment guides appear automatically'],
              ['⊸', 'Libavoid Routing',      'Route button — edges route around node obstacles'],
              ['◎', 'Node Position Anim',    'Smooth pan + zoom when selecting a node'],
              ['▣', 'Parent-Child Groups',   'Group button wraps nodes in a resizable frame'],
              ['✓', 'Remove Attribution',    'RF Pro badge hidden via proOptions'],
              ['☁', 'Server-Side Image',     'SVG button — server renders the full canvas'],
              ['◆', 'Shapes',                'Node shape varies by kind: circle · diamond · dashed'],
              ['⊞', 'Dynamic Layouting',     'ELK re-routes edges automatically on node add'],
            ].map(([icon, name, how]) => (
              <div key={name} style={{ display: 'flex', alignItems: 'flex-start', gap: 6, padding: '3px 0' }}>
                <span style={{ fontSize: 11, color: '#4ade80', flexShrink: 0, width: 16 }}>{icon}</span>
                <div>
        ...
        {/* ── Status bar ── */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 10, color: '#64748b', paddingLeft: 2 }}>
          <span style={{ color: isOptimizing ? '#34d399' : isLocked ? '#fbbf24' : isConnecting ? '#38bdf8' : '#64748b' }}>
            {statusText}
          </span>
          ...
          <span style={{ color: collaborationOnline ? '#34d399' : '#334155' }}>
...
export default function CanvasSurface({ onOpenInspector }) {
  ...
  const implementedProFeatureKeys = useMemo(() => new Set([
    // These map directly to React Flow Pro examples already implemented in this app.
    'auto-layout',
    'collaborative',
    'copy-paste',
    'dynamic-layouting',
  ...
  const reactFlowFeatureCoverage = useMemo(
    () => REACT_FLOW_PRO_EXAMPLES.map((feature) => ({
      ...feature,
      enabled: implementedProFeatureKeys.has(feature.key),
    })),
    [implementedProFeatureKeys],
  );

  useEffect(() => {
    if (hasDuplicateProFeatureEntries) {
      console.warn('[CanvasSurface] Duplicate React Flow Pro feature entries detected in REACT_FLOW_PRO_EXAMPLES.');
    }
  }, []);

  const implementedFeatureCount = useMemo(
    () => reactFlowFeatureCoverage.filter((feature) => feature.enabled).length,
    [reactFlowFeatureCoverage],
  );

  const implementedFeaturePercent = useMemo(
    () => Math.round((implementedFeatureCount / REACT_FLOW_PRO_EXAMPLES.length) * 100),
    [implementedFeatureCount],
  );
